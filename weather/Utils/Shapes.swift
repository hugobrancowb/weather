...
struct Arc: Shape {
  func path(in rect: CGRect) -> Path {
    ...
    path.move(to: CGPointMake(rect.minX-1, rect.minY))
    path.addQuadCurve(to: CGPointMake(rect.maxX+1, rect.minY), control: CGPointMake(rect.midX, rect.midY))
    path.addLine(to: CGPointMake(rect.maxX+1, rect.maxY+1))
    path.addLine(to: CGPointMake(rect.minX-1, rect.maxY+1))
    path.closeSubpath()
